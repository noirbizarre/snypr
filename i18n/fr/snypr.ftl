...
error-no-focused-output = aucune fenêtre n'est actuellement activée, ou son moniteur n'est pas encore connu
...
error-unsupported-compositor = aucune IPC de gestionnaire de fenêtres prise en charge n'a été détectée sur ce compositeur ; cette fonctionnalité n'est pas disponible ici
error-active-window-no-geometry = la fenêtre active a été trouvée, mais l'IPC du gestionnaire de fenêtres de ce compositeur ne signale ni sa position ni sa taille ; il n'y a donc rien pour recadrer la capture
