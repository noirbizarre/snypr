...
error-no-focused-output = no toplevel is currently activated, or its output is not yet known
...
error-unsupported-compositor = no supported window manager IPC was detected on this compositor; this feature is unavailable here
error-active-window-no-geometry = the active window was found, but this compositor's window-manager IPC does not report its position or size, so there is nothing to crop the capture to
